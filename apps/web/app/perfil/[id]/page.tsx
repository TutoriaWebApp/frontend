...
export default async function ProfilePage({
  ...
  const resultsTutor = await GetSpecificTutor(id);
  ...
  const resultsUserData = await GetSpecificUserData(tutorData?.usuarioId);
    ...
    <>
      {notAvailable && (
        <h1 className="mx-auto my-auto md:text-xl">
          Não foi possível obter os dados do tutor.
        </h1>
      )}
      {!notAvailable && (
