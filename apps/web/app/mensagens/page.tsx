...
export default function ChatPage() {
  ...
  const scrollToBottom = (behavior: ScrollBehavior = "auto") => {
    if (chatContainerRef.current) {
      ...
      if (activeChatId && qtdNaoLidasAtivo > 0) {
        marcarChatComoLido(activeChatId);
      }
    }
  };
  ...
  // Polling a cada 2s: mantém o estado das mensagens sem rolar a tela
  useEffect(() => {
    const intervalId = setInterval(async () => {
      ...
      if (chatsRes.success && chatsRes.data) {
        setChats((prevChats) => {
          return chatsRes.data!.map((novoChat) => {
            if (novoChat.id === activeChatId && isNearBottomRef.current) {
              return { ...novoChat, mensagensNaoLidas: 0 };
            }
            return novoChat;
          });
        });
      ...
      if (activeChatId) {
        const msgRes = await GetChatMessages(activeChatId, 1);
  ...
  }, [activeChatId, marcarChatComoLido]);

  // ROLAGEM: Executada estritamente ao entrar em um chat
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      const [msgRes, chatsRes] = await Promise.all([
        GetChatMessages(activeChatId, 1),
        GetChats(),
      ]);
  ...
  const handleScroll = async () => {
    ...
    if (isAtBottom && qtdNaoLidasAtivo > 0) {
      marcarChatComoLido(activeChatId);
    }
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden relative">
          ...
          <aside
            className={`
              ...
              transition-transform duration-200 ease-in-out md:translate-x-0 z-10
                ...
                return (
                  ...
                  >
                    <div className="relative shrink-0">
                      <div className="w-10 h-10 border border-slate-200 rounded-full flex items-center justify-center bg-slate-200 overflow-hidden">
                        {chat.fotoURL ? (
                          <img
                            src={chat.fotoURL}
                            alt={chat.nomePessoa}
                      ...
                      {temNaoLidas && (
                        <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-red-600 border-2 border-white rounded-full animate-pulse" />
                      )}
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex justify-between items-center gap-2 mt-0.5">
                        ...
                        {temNaoLidas && (
                          <span className="bg-red-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0 min-w-5 text-center shadow-xs">
              ...
              <>
                ...
                {loadingChatMessages ? (
                  ...
                  <div className="flex-1 min-h-0 relative flex flex-col">
                    ...
                    {/* Botão Flutuante: Sincronizado com o número da barra lateral */}
                    {qtdNaoLidasAtivo > 0 && (
                      <button
