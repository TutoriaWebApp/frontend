...
import ModalAgendamento from "@repo/ui/modal-agendamento/modalAgendamento";
...
export default function Tutores() {
  ...
  const [modalTutorId, setModalTutorId] = useState<number | null>(null);
    ...
    const fetchAllTutorData = async () => {
      ...
      try {
        const tutoresResponse = await fetch(
          "http://localhost:8000/v1/tutores/",
          {
        ...
        const userProfilePromises = tutoresData.map((tutor) => {
          return fetch(`http://localhost:8000/v1/usuarios/${tutor.usuarioId}`, {
            ...
            if (!res.ok) {
              throw new Error(
                `Falha ao buscar dados do usuário ${tutor.usuarioId}.`
              );
            }
            return res.json();
          });
        ...
        const finalUserProfiles: Usuario[] =
  ...
  const handleCloseModal = () => {
    setModalTutorId(null);
  };

  if (loading) {
    return (
      <div className="achievements-status">
        <h1>Carregando tutores...</h1>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="achievements-status error">
        <h1>Erro: {error}</h1>
      </div>
    );
  }
        ...
        <div className="teachers-grid">
          {tutores.map((tutor) => (
            <TeacherCard
              ...
              estado={tutor.estado}
              cidade={tutor.cidade}
              onAgendarClick={handleOpenModal}
            />
          ))}
        </div>
      </div>
      {modalTutorId !== null && (
        <ModalAgendamento tutorId={modalTutorId} onClose={handleCloseModal} />
      )}
    </>
  );
}
